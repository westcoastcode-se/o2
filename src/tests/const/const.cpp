...
void const_()
{
	...
	suite("const", []()
	{
		test("const_global", ROOT_PATH, [](syntax_tree& st)
		{
			...
			const auto ref_INT2 = assert_type<node_type_known_ref>(var_INT2->get_type());
			...
			const auto ref_FLOAT2 = assert_type<node_type_known_ref>(var_FLOAT2->get_type());
			...
			const auto ref_FLOAT3 = assert_type<node_type_known_ref>(var_FLOAT3->get_type());
		...
		test("const_func", ROOT_PATH, [](syntax_tree& st)
		{
			...
			const auto ret_ref_int = assert_type<node_type_known_ref>(ret->get_child(0));
